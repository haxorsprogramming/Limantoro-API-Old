...
<html>
<head>
	<title>Laporan Pengembalian Pembelian</title>
	...
	<header style="">
		<table class="tbl">
			<tr>
				...
				<td rowspan="3" class="tbl-title"> Pengembalian Pembelian </td>
			</tr>
			<tr>
	 ...
	 <main>
		...
		<table style="width:100%;">
			<tr>
			<td class="bold txt-left" style="width:115px;">No. PRN</td>
				<td class="two_dot">:</td>
				<td class="txt-left">{{$data->number}}</td>
				<td class="bold txt-left" style="width:115px;">No. PO</td>
				<td class="two_dot">:</td>
				<td class="txt-left">{{$data->purchase_order_number}}</td>
				<td class="bold txt-left" style="width:115px;">Tanggal Dibuat</td>
				<td class="two_dot">:</td>
		...
		<table class='table-bordered table-striped list'>
			...
			<tbody>
				@php $i=1 @endphp
				@foreach($data->purchase_return_details as $purchase_return_detail)
				<tr>
					<td>{{ $i++ }}</td>
					<td>{{$purchase_return_detail->material->code}}</td>
					<td>{{$purchase_return_detail->material->name}}</td>
					<td>{{$purchase_return_detail->material->satuan}}</td>
					<td>{{writeIDFormat($purchase_return_detail->qty)}}</td>
				</tr>
				@endforeach
			</tbody>
			{{--<tfoot>
		...
		<table style="width:100%; margin-top:15px;">
			<tr>
				...
				<td style="width:150px; text-align:center;">
					...
					<br>
					<br>
					{{$data->purchase_order->purchase_request->project->in_charge->name}}
				</td>
				<td style="width:150px; text-align:center;">
					Dibuat Oleh
					<br>
				...
				<td style="width:150px; text-align:center;">
					...
					<br>
					<br>
					{{$data->purchase_order->purchase_request->approver->name}}
				</td>
			</tr>
		</table>
	</main>
