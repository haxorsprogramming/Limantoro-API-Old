	...
	<header style="">
		<table class="tbl">
			<tr>
				...
				<td rowspan="3" class="tbl-title"> Pengembalian Barang </td>
			</tr>
			<tr>
	 ...
	 <main>
		...
		<table style="width:100%;">
			<tr>
				<td class="bold txt-left" style="width:115px;">No. GRN</td>
				<td class="two_dot">:</td>
		...
		<table class='table-bordered table-striped list'>
			...
			<tbody>
				@php $i=1 @endphp
				@foreach($data->goods_return_details as $goods_return_detail)
				<tr>
					<td>{{ $i++ }}</td>
					<td>{{$goods_return_detail->material->code}}</td>
					<td>{{$goods_return_detail->material->name}}</td>
					<td>{{$goods_return_detail->material->satuan}}</td>
					<td>{{writeIDFormat($goods_return_detail->qty)}}</td>
				</tr>
				@endforeach
			</tbody>
			{{--<tfoot>
