	...
	<header style="">
		<table class="tbl">
			<tr>
				...
				<td rowspan="3" class="tbl-title"> Penerimaan Barang </td>
			</tr>
			<tr>
	 ...
	 <main>
		...
		<table style="width:100%;">
			<tr>
			<td class="bold txt-left" style="width:115px;">No. GR</td>
				<td class="two_dot">:</td>
				...
				<td class="bold txt-left" style="width:115px;">Tanggal Diterima</td>
				<td class="two_dot">:</td>
				<td class="txt-right" style="width:160px;">{{dateFormalID($data->date)}}</td>
			</tr>
			<tr>
				<td class="bold txt-left">No Surat Jalan</td>
				<td class="two_dot">:</td>
				<td class="txt-left">{{$data->delivery_order_letter_number}}</td>
		...
		<table class='table-bordered table-striped list'>
			...
			<tbody>
				@php $i=1 @endphp
				@foreach($data->goods_receipt_details as $goods_receipt_detail)
				<tr>
					<td>{{ $i++ }}</td>
					<td>{{$goods_receipt_detail->material->code}}</td>
					<td>{{$goods_receipt_detail->material->name}}</td>
					<td>{{$goods_receipt_detail->material->satuan}}</td>
					<td>{{writeIDFormat($goods_receipt_detail->qty)}}</td>
				</tr>
				@endforeach
			</tbody>
			{{--<tfoot>
